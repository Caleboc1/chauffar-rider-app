...
type DriverPickupMapProps = {
  request: DriverRideRequest;
  waitingForPassenger: boolean;
  onArrival: () => void;
  onProgressChange: (progress: number) => void;
};

const TOTAL_TRAVEL_DURATION_MS = 9000;
const MIN_SEGMENT_DURATION_MS = 850;
const MAX_SEGMENT_DURATION_MS = 1900;
...
export function DriverPickupMap({
  ...
  const routeCoordinates = useMemo(() => request.pickupPreview.routeCoordinates, [request.pickupPreview.routeCoordinates]);
  ...
  useEffect(() => {
    if (waitingForPassenger) {
      setCompletedPointIndex(denseRouteCoordinates.length - 1);
      animatedCoordinate.setValue({
        latitude: request.pickupPreview.passengerLocation.latitude,
        longitude: request.pickupPreview.passengerLocation.longitude,
        latitudeDelta: 0,
        longitudeDelta: 0,
      });
      onProgressChange(1);
    ...
    onArrival,
    onProgressChange,
    request.pickupPreview.passengerLocation.latitude,
    request.pickupPreview.passengerLocation.longitude,
    segmentDurations,
    waitingForPassenger,
  ]);
  ...
  const region = useMemo(() => request.pickupPreview.region, [request.pickupPreview.region]);

  const isNearPassenger = waitingForPassenger || completedPointIndex >= denseRouteCoordinates.length - 4;
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      ...
      >
        ...
        <Marker
          coordinate={request.pickupPreview.passengerLocation}
          anchor={{ x: 0.5, y: 0.5 }}
          tracksViewChanges={false}
        >
          <PassengerMarker arrived={isNearPassenger} />
        </Marker>
      </MapView>
    </View>
  );
